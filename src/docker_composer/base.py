...
def check_v2():
    ...
    logger.debug(p.stdout.strip().decode("UTF-8"))
    return "Docker Compose version" in p.stdout.strip().decode("UTF-8")
...
class DockerBaseRunner:
    ...
    def _call_cmd(self, arguments: Iterable[str] = ()) -> List[str]:
        logging_level = os.environ.get("DOCKER_COMPOSE_LOG_LEVEL", "info")
